...
class Purchase(models.Model):
    ...
    user = models.ForeignKey(
        UserProfile, on_delete=models.PROTECT, related_name="purchase"
    )
    ...
    @property
    def total(self):
        total_price = models.Sum(models.F("amount") * models.F("book__price"))
        queryset = self.itens.all().aggregate(total=total_price)
        return queryset["total"]


class ItemPurchase(models.Model):
    purchase = models.ForeignKey(
        Purchase, on_delete=models.CASCADE, related_name="itens"
    )
